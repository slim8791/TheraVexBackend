...
public class FacturesController : ControllerBase
{
    ...
    // ---------------- CREATE BON COMMANDE ----------------
    [HttpPost("boncommande")]
    public async Task<IActionResult> CreateBonCommande(BonCommandeePdfDto dto)
    {
        using var transaction = await _context.Database.BeginTransactionAsync();
        var lastDocument = _context.BonCommandes.OrderByDescending(f => f.Numero).FirstOrDefault();
        string numDoc = lastDocument == null ? "00001" : (lastDocument.Numero++).ToString("00000");
        try
        {
            var facture = new BonCommande
            {
                FournisseurId = int.Parse(dto.Fournisseur),
                Numero = int.Parse(numDoc),
                FullNumber = $"BC-{numDoc}-{DateTime.Now:yyyy}",
                Date = dto.Date,
            ...
            foreach (var l in dto.Lignes)
            {
                ...
                decimal prix = l.PrixUnitaire != 0 ? l.PrixUnitaire : article.PrixVente;
                decimal ligneHT = l.Quantite * prix;
                decimal ligneTVA = ligneHT * (article.Tva.Taux / 100);


                facture.Lignes.Add(new BonCommandeLigne
                {
                    ArticleId = article.Id,
                    Quantite = l.Quantite,
                    PrixUnitaire = prix,
            ...
            _context.BonCommandes.Add(facture);
            await _context.SaveChangesAsync();
            await transaction.CommitAsync();
        ...
        {
            await transaction.RollbackAsync();
            return BadRequest("Erreur lors de la création du bon de commande");
        }
    }

    // ---------------- GENERATION PDF ----------------
    [HttpGet("{id}/boncommande/pdf")]
    public async Task<IActionResult> GetBonCommandePdf(int id)
    {
        ...
        var facture = await _context.BonCommandes
            .Include(f => f.Fournisseur)
            .Include(f => f.Lignes)
            .ThenInclude(l => l.Article)
            .ThenInclude(a => a.Tva)
            .FirstOrDefaultAsync(f => f.Id == id);
        List<BonCommandeLigneVM> factureLigneVMs = new List<BonCommandeLigneVM>();
        foreach (var item in facture.Lignes)
        {
            ...
            {
                case 7:
                    totalTva7 += (item.PrixUnitaire * item.Quantite) * (item.TauxTva / 100);
                    tva7 += item.PrixUnitaire * item.Quantite;
                    break;
                case 19:
                    totalTva19 += (item.PrixUnitaire * item.Quantite) * (item.TauxTva / 100);
                    tva19 += item.PrixUnitaire * item.Quantite;
                    break;
            }

            totalTVA += (item.PrixUnitaire * item.Quantite * (item.TauxTva / 100));
            totalHT += item.PrixUnitaire * item.Quantite;
            totalAvantRemise += item.PrixUnitaire * item.Quantite;

            factureLigneVMs.Add(new BonCommandeLigneVM
            {
        ...
        var client = await _context.Fournisseur
            .FirstOrDefaultAsync(c => c.FournisseurId == facture.FournisseurId);

        BonCommandeViewModel fvm = new BonCommandeViewModel
        {
            FournisseurNom = client.RaisonSociale,
            Numero = facture.FullNumber,
            FournisseurAdresse = client.Adresse,
            FournisseurMF = client.MatriculeFiscale,
            FournisseurTel = client.Telephone.ToString(),
            Date = DateTime.Now,
        ...
        fvm.TotalTTC = fvm.TotalHT + fvm.TotalTVA;
        fvm.NetAPayer = fvm.TotalTTC;
        var html = await _renderer.RenderViewToStringAsync("BonCommande", fvm);
        ...
        string fileName = $"Bon_Commande_{client.RaisonSociale}_{facture.FullNumber}.pdf";
        string saveDir = Path.Combine(Directory.GetCurrentDirectory(), "GeneratedPdfs", "BonCommande", client.RaisonSociale);
